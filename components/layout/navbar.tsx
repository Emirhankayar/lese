...
const routeList: { href: string; label: string; icon?: React.ReactNode }[] = [
  ...
  { href: "/profile", label: "Profil", icon: <UserIcon className="w-4 h-4" /> },
];
...
export const Navbar = () => {
  ...
  // Don't show loading spinner - let the navbar render immediately
  // Supabase will quickly restore the session if it exists
  if (loading) {
    return (
